...
fn test_sum() {
    ...
    // dfdx fp32 as ground truth (same as Luminal CPU, verified exact match)
    let d_dev = Cpu::default();
    ...
    // Metal fp16 should match dfdx fp32 ground truth closely
    // Metal computes in fp16 but converts result back to f32
    // Observed max diff: ~0.007-0.03 for axis 2 (4096 elements)
    assert_close_precision(&b.data(), &d_b.as_vec(), 0.05);
    assert_close_precision(&c.data(), &d_c.as_vec(), 0.01);
    assert_close_precision(&d.data(), &d_d.as_vec(), 0.01);
}
...
fn test_sum2() {
    ...
    // Use relaxed precision for fp16 sum reductions (exact equality is unrealistic)
    assert_close_precision(&d.data(), &d_d.to_dtype::<f32>().as_vec(), 1e-2);
}
...
fn test_conv2d() {
    ...
    cx_metal.compile(MetalCompiler::<f16>::default(), &mut out_metal);
    cx_metal.execute();
    ...
    assert_close_precision(&metal_result, &cpu_result, 1e-2);
}

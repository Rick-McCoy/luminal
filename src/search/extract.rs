...
use super::debug::display_graph;
#[cfg(any(feature = "cuda", feature = "metal"))]
use super::run::{assign_buffers, compile_kernels, htod, new_buffer, run_graph};
use super::translate::InitData;
#[cfg(any(feature = "cuda", feature = "metal"))]
use super::utils::{build_search_space, print_kernels};
#[cfg(any(feature = "cuda", feature = "metal"))]
use super::Buffer;
#[cfg(any(feature = "cuda", feature = "metal"))]
use super::GPUArch;
...
pub fn search(
        ...
        .map(|(n, init)| {
            (
                ...
                match init {
                    ...
                    InitData::Expr(e) => htod(&[e.exec(dyn_vars).unwrap() as f32], &device),
                },
            )
        })
        .collect_vec();
    ...
    if option_env!("DEBUG").is_none() {
        ui_functions = Some(super::utils::search_ui());
    };
    ...
    'trajectory_loop: for (n, trajectory) in trajectories.into_iter().enumerate() {
        ...
        let Some((kernels, gmem_mapping)) =
            super::codegen::codegen(graph.clone(), arch.clone(), dyn_vars)
        else {
            continue;
        };
        valids += 1;
                    ...
                    for (x, y) in a.iter().zip(b) {
                        if (x - y).abs() >= 1e-4 {
                            if option_env!("DEBUG").is_some() {
                                ...
                                super::debug::display_multiple_graphs(&[&ref_graph, &graph]);

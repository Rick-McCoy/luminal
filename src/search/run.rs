...
pub fn run_graph(
    ...
    intermediate_buffers: &mut [Buffer],
    ...
    for node in toposort(kernels, None).unwrap() {
        ...
        if kernel.code == "Inputs" {
            ...
            let data: Vec<f32> = stream.clone_dtoh(buffer).unwrap();
        ...
        for node in toposort(kernels, None).unwrap() {
            ...
            if kernel.code == "Inputs" {
                ...
                let (input, input_index) = kernels
                    ...
                    .unwrap();
                let buffer = &intermediate_buffers[intermediate_buffer_map[&input][input_index]];
                let mut data = vec![0_f32; buffer.length() / size_of::<f32>()];
                unsafe {
...
pub fn dtoh(buf: &CudaSlice<f32>) -> Vec<f32> {
    buf.stream().clone_dtoh(buf).unwrap()
}

#[cfg(feature = "metal")]
pub fn htod(v: &[f32], device: &Device) -> Buffer {
    assert!(!v.is_empty());
    unsafe {
...
pub fn dtoh(v: &Buffer) -> Vec<f32> {
    let mut data = vec![0f32; v.length() / size_of::<f32>()];
